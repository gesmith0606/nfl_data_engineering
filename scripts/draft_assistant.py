...
def _load_rostered_file(path: str) -> list:
    ...
    if not os.path.exists(path):
        print(
            f"WARNING: --rostered-file '{path}' not found; waiver list will be empty."
        )
        return []
...
def run_simulation(
    ...
    simulator = MockDraftSimulator(
        board, user_pick=my_pick, n_teams=n_teams, randomness=randomness
    )
...
"""
    ...
    while True:
        ...
        prompt_prefix = f"[Budget: ${summary['budget_remaining']} | Spots: {summary['spots_remaining']} left]"
        ...
        if cmd in ("quit", "q", "exit"):
            ...
            fair_cost = (
                projected / board._league_avg_pts_per_dollar
                if board._league_avg_pts_per_dollar > 0
                else 0
            )
            _sep()
            ...
            pts = player.get(
                "projected_season_points", player.get("projected_points", "?")
            )
            pts_str = f"{pts:.1f}" if isinstance(pts, float) else str(pts)
            print(
            ...
            print(
                f"  Roster Spots Filled : {summ['roster_spots_filled']} / {summ['roster_spots_total']}"
            )
            ...
            if not info:
                ...
                print(
                    f"  Assessment       : {overpay_label} ({info['overpay_pct']:+.1f}% vs fair value)"
                )
...
def run_draft_session(
    ...
    _header(
        f"FANTASY DRAFT ASSISTANT  |  {scoring_format.upper()}  |  {n_teams}-Team  |  Pick #{my_pick}"
    )
...
"""
    ...
    while True:
        ...
        prompt_label = (
            "YOUR PICK" if is_my_turn else f"Pick #{pick_number} (Round {round_number})"
        )
        ...
        if cmd in ("quit", "q", "exit"):
            ...
            if "player_name" in board.available.columns:
                mask = (
                    board.available["player_name"]
                    .str.lower()
                    .str.contains(arg.lower(), na=False)
                )
                results = board.available[mask]
                _display_players(results, max_rows=10)
            pick_number -= 1
...
def main():
    ...
    if args.projections_file and os.path.exists(args.projections_file):
        ...
        try:
            ...
            print(
                "Tip: Run 'python scripts/generate_projections.py --preseason --season 2026' first"
            )
            return 1
    ...
    # -----------------------------------------------------------------------
    # Attach season-market provenance columns (never blends the projections)
    # Latest DK season futures snapshot -> prop_implied_points (market's
    # implied season total) + prop_anchor_gap (model - market). A large gap
    # is a "form an opinion before paying the model's price" flag on the
    # draft board. Missing snapshot -> columns stay NaN, board unchanged.
    # -----------------------------------------------------------------------
    try:
        from season_prop_implied import attach_market_columns

        projections, matched = attach_market_columns(
            projections,
            season=args.season,
        ...
        if matched:
            print(
                f"Market lines attached: {matched} players with DK season-"
                "futures implied points (prop_anchor_gap = model - market)"
            )
    except Exception as exc:
        print(f"WARN: Could not attach market columns: {exc}")
    ...
    if args.rostered_file:
        ...
        if rostered_players:
            print(
                f"Loaded {len(rostered_players)} rostered players from '{args.rostered_file}'"
            )
