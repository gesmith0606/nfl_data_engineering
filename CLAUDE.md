...
**In progress**: TPRR slope-collapse production wiring (lab-gated +0.0275 WR MAE; requires blend-consistent WR residual retrain — see .planning/RANK_ORDERING_EXPERIMENTS.md) | Website sentiment integration (news router wired, blocked on ANTHROPIC_API_KEY) | Rebuild wr_matchup_*/te_matchup_* graph features as lagged trailing versions (currently excluded as same-game leaks)

**Planned**: Prop-implied projections (BUY memo .planning/PROP_IMPLIED_DECISION.md, needs ODDS_API_KEY) | v5.0 Sentiment multiplier wiring | v5.1 Perfect implementation (Neo4j Aura, PFF subscription) | v5.2 Live data sync
