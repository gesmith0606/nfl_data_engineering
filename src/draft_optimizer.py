...
def compute_value_scores(
    ...
    if adp_df is not None and not adp_df.empty:
        ...
        # ``stdev`` (per-player ADP standard deviation) is optional -- present
        # once Lane 1's Fantasy Football Calculator source lands, absent from
        # today's data. Carry it through as ``adp_stdev`` when available so
        # downstream consumers (gone-probability, tiering) can use real
        # per-player spread instead of a fallback; degrades to no column
        # (callers fall back on their own) when absent.
        has_stdev = "stdev" in adp_df.columns
        cols = [join_col, "adp_rank"] + (["stdev"] if has_stdev else [])
        adp_subset = adp_df[cols].copy()
        if has_stdev:
